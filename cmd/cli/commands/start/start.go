...
func New(cm *config.ConfigManager) *start {
	return &start{cm}
}
...
func serviceRun(ctx context.Context, configPath string, payload startServicePayload) (startOutputPayload, error) {
...
func (s *start) repoRun(ctx context.Context, payload startInputPayload) (startOutputPayload, error) {
	res := startOutputPayload{
		Pid:     -1,
		RetCode: -1,
	}
	...
	denv, _ := godotenv.ReadFrom(payload.gitPath, false, s.cm.EnvFilenames...)
	startProcess := shell.NewSyncShell(shell.ShellOps{
...
func (s *start) Command() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			startServiceFn := func(payload startServicePayload) (startOutputPayload, error) {
				return serviceRun(onStartCtx, path.Dir(s.cm.ConfigPath), payload)
			}
			...
			startFn := func(args startInputPayload) (startOutputPayload, error) {
				return s.repoRun(onStartCtx, args)
			}
			...
			stopServiceFn := func(payload startServicePayload) (startOutputPayload, error) {
				return serviceRun(nil, path.Dir(s.cm.ConfigPath), payload)
			}
			...
			stopFn := func(args startInputPayload) (startOutputPayload, error) {
				return s.repoRun(onStopCtx, args)
			}
			for _, repo := range executableRepo {
